...
class PlayState extends MusicBeatState
{
	...
	// red shit
	var redbg:FlxSprite;
	var pixelbg:FlxSprite;
	var chadizard:FlxSprite;
	var pikachu:FlxSprite;
	var pixelchadizard:FlxSprite;
	var pixelpikachu:FlxSprite;
	var pokehealth:FlxSprite;
	...
	override public function create()
	{
			...
			case 'redstage':
				{
					defaultCamZoom = 0.7;
					curStage = 'redstage';
					redbg = new FlxSprite(0, 0).loadGraphic(Paths.image('red/fondo_pokemon_1'));
					redbg.antialiasing = true;
					redbg.scale.set(1.2, 1.2);
					redbg.active = false;
					add(redbg);

					pixelbg = new FlxSprite(0, 0).loadGraphic(Paths.image('red/fondo_pokemon_2'));
					pixelbg.scale.set(1.2, 1.2);
					pixelbg.antialiasing = true;
					//bg.scrollFactor.set(0.9, 0.9);
					pixelbg.active = false;
					pixelbg.visible = false;
					add(pixelbg);

					var pokeTex = Paths.getSparrowAtlas('red/charizard');
					chadizard = new FlxSprite(650, -250);
					chadizard.frames = pokeTex;
					chadizard.setGraphicSize(Std.int(chadizard.width * 0.7));
					chadizard.animation.addByPrefix('idle', "chari idle", 24);
					chadizard.animation.addByPrefix('atack', "charizard atack", 24);
					chadizard.animation.play('idle');
					chadizard.antialiasing = true;
					chadizard.visible = true;
					add(chadizard);
					

					var pikaTex = Paths.getSparrowAtlas('red/pikachu');
					pikachu = new FlxSprite(1300, 200);
					pikachu.frames = pikaTex;
					pikachu.setGraphicSize(Std.int(pikachu.width * 0.7));
					pikachu.animation.addByPrefix('idle', "PIKACHU IDLE", 24);
					pikachu.animation.addByPrefix('atack', "pikaatack", 24);
					pikachu.animation.play('idle');
					pikachu.antialiasing = true;
					//pikachu.visible = true;
					//add(pikachu);

					var pokeTex2 = Paths.getSparrowAtlas('red/pixelcharizard');
					pixelchadizard = new FlxSprite(800, 0);
					pixelchadizard.frames = pokeTex2;
					pixelchadizard.setGraphicSize(Std.int(pixelchadizard.width * 0.7));
					pixelchadizard.animation.addByPrefix('idle', "chariidle instancia 1", 24);
					pixelchadizard.animation.play('idle');
					pixelchadizard.antialiasing = false;
					pixelchadizard.visible = false;
					add(pixelchadizard);

					var pikaTex2 = Paths.getSparrowAtlas('red/pixelpikachu');
					pixelpikachu = new FlxSprite(1300, 300);
					pixelpikachu.frames = pikaTex2;
					pixelpikachu.setGraphicSize(Std.int(pixelpikachu.width * 0.7));
					pixelpikachu.animation.addByPrefix('idle', "pikachu idle instancia 1", 24);
					pixelpikachu.animation.play('idle');
					pixelpikachu.antialiasing = false;
					pixelpikachu.visible = false;
					//add(pixelpikachu);

					pokehealth = new FlxSprite (-50, 400).loadGraphic(Paths.image('red/health'));
					if(!PlayStateChangeables.useDownscroll)
					{
						pokehealth.x = 225;
					    pokehealth.y = 360;
					}	
					pokehealth.antialiasing = false;
					pokehealth.scale.set(0.4, 0.4);
					pokehealth.cameras = [camHUD];
					pokehealth.active = false;
		...
		switch (curStage)
		{
			...
			case 'redstage':
				...
				boyfriend.x -= 700; //70
				boyfriend.y += 120; //570 checkpoint
			case 'redstage-pixel':
				//boyfriend.scale.set(1.3, 1.3);
				boyfriend.x -= 600; //170
				boyfriend.y += 50; //500
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				songPosBG = new FlxSprite(0, 10).loadGraphic(Paths.image('healthBarOld'));
				if (PlayStateChangeables.useDownscroll)
		...
		healthBarBG = new FlxSprite(-25, 510).loadGraphic(Paths.image('healthBar'));
		if(!PlayStateChangeables.useDownscroll)
		{
			healthBarBG.x = 250;
		    healthBarBG.y = 470;
		}
		healthBarBG.scale.set(0.4, 0.4);
		healthBarBG.scrollFactor.set();
		add(healthBarBG);

		healthBar = new FlxBar(healthBarBG.x + 223, healthBarBG.y + 86, LEFT_TO_RIGHT, Std.int(healthBarBG.width - 408), Std.int(healthBarBG.height - 190), this,
			'health', 0, 2);
		...
		kadeEngineWatermark = new FlxText(4, FlxG.height * 0.9 + 45,0,SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | KE " + MainMenuState.kadeEngineVer : ""), 16);
		...
		if (PlayStateChangeables.useDownscroll)
			scoreTxt = new FlxText(FlxG.width / 2 - 235, 60, 0, "", 20);
		else
			scoreTxt = new FlxText(FlxG.width / 2 - 235, FlxG.height * 0.9, 0, "", 20);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			if (section.altAnim && curStage == "redstage" && FlxG.save.data.distractions)
				noteStyle = 'pixel';
	...
	override public function update(elapsed:Float)
	{
		...
		{
			var lowhp = FlxG.sound.play(Paths.music('lowhp'), 300);
		}
	...
	var pixelShitPart1:String = ''; //prueba
	...
	private function popUpScore(daNote:Note):Void
		{
				...
				case 'shit':
					switch(daNote.noteType)
					{
						case 3:
						{
							...
							ss = false;
							misses++;
							if (!pixelpikachu.visible)
							{
								pikachu.visible = false;
								var pikachuangry:FlxSprite = new FlxSprite(pikachu.x, pikachu.y);
								pikachuangry.frames = Paths.getSparrowAtlas('red/pikachu');
								pikachuangry.animation.addByPrefix('attack','pikaatack',24,false);
								pikachuangry.setGraphicSize(Std.int(pikachuangry.width * 0.7));
								add(pikachuangry);
								pikachuangry.animation.play('attack');
								pikachuangry.animation.finishCallback = function(name:String) {
									health -= 0.4;
									remove(pikachuangry);
									pikachu.visible = true;	
								}
						...
						case 2:
						{
							...
							ss = false;
							misses++;
							if (!pixelchadizard.visible)
							{
								chadizard.visible = false;
								var chadizardangry:FlxSprite = new FlxSprite(chadizard.x - 300, chadizard.y - 20);
								chadizardangry.frames = Paths.getSparrowAtlas('red/charizard');
								chadizardangry.animation.addByPrefix('attack','charizard atack',24,false);
								chadizardangry.setGraphicSize(Std.int(chadizardangry.width * 0.7));
								add(chadizardangry);
								chadizardangry.animation.play('attack');
								chadizardangry.animation.finishCallback = function(name:String) {
									health -= 0.4;
									remove(chadizardangry);
									chadizard.visible = true;
								}
							}
							else
			...
			if ( curStage.startsWith('school') || curStage.startsWith('redstage-pixel'))
			{
			...
			if (isPixelUI)
				comboSpr = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'combo' + pixelShitPart2, 'week6'));
			else
			...
			for (i in seperatedScore)
			{
				...
				if (isPixelUI)
				    numScore = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'num' + Std.int(i) + pixelShitPart2, 'week6'));
				else
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					!PlayStateChangeables.useDownscroll && daNote.y < strumLine.y)
					{
						...
						if(PlayStateChangeables.botPlay && daNote.canBeHit && daNote.mustPress && daNote.noteType == 0||
						PlayStateChangeables.botPlay && daNote.tooLate && daNote.mustPress && daNote.noteType == 0)
						{
							if(loadRep)
							{
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase().startsWith('pokemon') && FlxG.save.data.distractions)
		{
			switch(curStep)
			{
				case 512 | 640 | 1056:
	...
	function becomefumo()
		{
			...
			remove(pixelpikachu);
			//remove(gf);
			dad = new Character(1130, -150, SONG.player2);
			dad.scale.set(0.8, 0.8);
			boyfriend = new Boyfriend(70, 570, SONG.player1);
			boyfriend.scale.set(1.3, 1.3);
